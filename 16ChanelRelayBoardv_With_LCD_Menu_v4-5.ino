...
int Version = 1;                    // update or read main version at EEPROM address 211
int SubVersion = 4.5;               // update or read main version at EEPROM address 212
...
int AccruTiming =0;                 // sued for adding the sequence time from timing to apply to cue 32 when processing  0 timing zero timing delays
...
void SetTempDelaysToZero() {
  for (int i = 1; i < 33; i = i + 1) {
    tempDelays[i] = 0;
 ("tempDelays ");  Serial.print(i);Serial.print(" = ");Serial.println(tempDelays[i]);
  }
}
...
//Only use this option if you have a 12dc relay board powered by a seperate power supply. do not power from Arduino
...
void CreateSequenceFromTiming() {
  ...
  int S = 2;
  sequence[0][0] = timing[1];                sequence[0][1] = cue1; sequence [0][2] = LOW;
  sequence[1][0] = timing[1] + delays[33];   sequence[1][1] = cue1; sequence [1][2] = HIGH;
  for (int i = 2; i < 32; i++) {
  Serial.print("Sequence ");Serial.print(i); Serial.print(" = ");Serial.println(sequence[1][0]); 
  if (timing[i] != timing[i-1] && timing[i] != 0) {if(AccruTiming<timing[i]){AccruTiming = timing[i];}
    sequence[S][0] = timing[S];                sequence[S][1] = cue2; sequence [S][2] = LOW;
    S=S+1;
    sequence[S][0] = timing[S-1] + delays[33];   sequence[S][1] = cue2; sequence [S][2] = HIGH;
  Serial.print("Sequence ");Serial.print(i); Serial.print(" = ");Serial.println(sequence[1][0]); 
  } else {
    sequence[S][0] = timing[S];   sequence[S][1] = NoCue; sequence [S][2] = LOW;
    S=S+1;
    sequence[S][0] = timing[S-1];   sequence[S][1] = NoCue; sequence [S][2] = LOW;
  Serial.print("Sequence ");Serial.print(i); Serial.print(" = ");Serial.println(sequence[1][0]); 
  }
  S=S+1;
  }
  if (timing[32] != timing[31] && timing[32] != 0) {if(AccruTiming<timing[32]){AccruTiming = timing[32];}
  sequence[62][0] = timing[32];              sequence[62][1] = cue32; sequence [62][2] = LOW;
  sequence[63][0] = timing[32] + delays[33]; sequence[63][1] = cue32; sequence [63][2] = HIGH;
    } else {
  sequence[62][0] = AccruTiming;              sequence[62][1] = cue32; sequence [62][2] = LOW;
  sequence[63][0] = AccruTiming + delays[33]; sequence[63][1] = cue32; sequence [63][2] = HIGH;
     }
  Serial.print("Sequence 32 = ");Serial.println(sequence[32][0]); 
  delay(10000);
...
lcd.setCursor(0, 3); lcd.print("STATUS = DISARMED");

RGB_color(0, 75, 0); // Green 0, 255, 0
...
//      Serial.println("btn push was pressed on main loop");
  ...
  if (armed == 1) {
    ...
    RGB_color(150, 75, 0); // Yellow 255,255,0
...
void page0() {
  ...
  if (Trigger == 1) {
    ...
    Serial.print("TriggerST = ");  Serial.println(TriggerSt);
  lcd.clear();
  //                              01234567890123456789
  lcd.setCursor(0, 0); lcd.print("---INFO SCREEN--- "); if(FireMode == 0){lcd.print(numOfCommonCues);}else{lcd.print("  ");} //battLevel();
  lcd.setCursor(0, 1); lcd.print("FIRE MODE ="); lcd.setCursor(12, 1); lcd.print(FireModeSt);
  ...
  switch (vpoz)
  {
    case 0:
      if (SelectKey == SelectKey_max) {
        SelectKey = 0;
        vpoz = 0;  page = 1;  RGB_color(0, 0, 75); // Blue 0,0,255
        lcd.clear();  page1();
      }
      break;
  }
...
void page1() {
  ...
  lcd.setCursor(0, 1); lcd.print(" TRIGGER TYPE = "); lcd.setCursor(16, 1); lcd.print(TriggerSt);
...
void page11() {
  ...
  lcd.setCursor(0, 1); lcd.print(" STEP"); if (TimeType == 0) {
    lcd.print(" RELATIVE");
  } if (TimeType == 1) {
    lcd.print(" ABSOLUTE");
  }
...
void page1111() {
  ...
  switch (vpoz)
  {
    ...
    case 6: // Exit
      lcd.setCursor(0, 3); lcd.print("*");
//        Serial.println("page 1111 case 6");
      if (SelectKey == SelectKey_max) {
  ...
  switch (vpoz) {
    case 0:
      ...
      if (SelectKey == SelectKey_max) {
        ...
        copyDisplayMillsToTempDelays();       // store the read values that were assigned to the displayMills array to Temp Delays array
        copyTempDelaysToDelays();             // Convert the tempDelays array data to delays array
        if(FireMode==0){                      // If Common Delay is set
          GetTimingValuesFromCommonDelays();  // create a timing based on the delays array data
        }
        if(FireMode==1){                      // If Step delay is set
          GetTimingValuesFromDelays();        // create a timing based on the delays array data
        }
//        copyTimingToTempTiming();
//        copyTempTimingToTimingDisplayMills();
...
void page1311() {
  ...
  lcd.setCursor(0, 2); lcd.print(" VERSION:  "); lcd.print(Version); lcd.print("."); lcd.print(SubVersion);
  ...
  switch (vpoz)
  {
    ...
    case 1:
      ...
      lcd.setCursor(15, 3); lcd.print(" ");
      if (SelectKey == SelectKey_max) {
        SelectKey = 0; vpoz = 0; page = 133; returnPage = 1311; returnVpoz = 1; lcd.clear(); page133();
      }
      break;
    case 2:
      lcd.setCursor(0, 0); lcd.print(" ");
    ...
    case 3:
      ...
      break;
    case -1:
      SelectKey = 2; vpoz = 0; page = 13; lcd.clear(); page13();
      break;
  }
}
